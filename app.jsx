    ...
    <>
      ...
      <div className="app">
        ...
        <div className="topbar glass">
          ...
          <button className="btn" onClick={() => setAudioSettingsOpen(true)} title="Audio Output Settings">
            <svg width="13" height="13" viewBox="0 0 13 13" fill="none" stroke="currentColor" strokeWidth="1.4">
              <path d="M1 4h3l2.5-2.5v10L4 9H1V4z"/>
              <path d="M8.5 5a2.5 2.5 0 010 3.5" opacity=".7"/>
              <path d="M10.5 3.5a5 5 0 010 6" opacity=".4"/>
            </svg>
            Audio Out
          </button>
      ...
      {audioSettingsOpen && <AudioSettingsModal onClose={() => setAudioSettingsOpen(false)}/>}
...
function SampleRow({ name, onDragStart, onDragEnd }) {
  ...
  const handlePreview = (e) => {
    ...
    if (previewing) {
      ...
      const isDrum = /kick|snare|hat|clap|tom|perc|rim|snap/i.test(name);
      setTimeout(() => setPreviewing(false), isDrum ? 350 : 1300);
    }
  };

  return (
    <div className="sample" draggable onDragStart={() => { stopPreview(); setPreviewing(false); onDragStart(name); }} onDragEnd={onDragEnd}>
      <button
        className={`preview-btn${previewing ? ' preview-btn-on' : ''}`}
      ...
      >
        {previewing
          ...
          : <svg width="8" height="8" viewBox="0 0 8 8"><path d="M1 1L7 4L1 7V1Z" fill="currentColor"/></svg>
        }
      </button>
      <span className="wave"/>
      {name}
    ...
    <>
      {folders.map((f, i) => (
        <div key={i} className={`folder ${f.open ? "open" : ""}`}>
          ...
          <div className="folder-items">
            {f.samples.map((s, j) => (
              <SampleRow key={j} name={s} onDragStart={onDragStart} onDragEnd={onDragEnd}/>
            ))}
          </div>
        </div>
      ))}
...
function AudioSettingsModal({ onClose }) {
  ...
  React.useEffect(() => {
    if (!navigator.mediaDevices?.enumerateDevices) { setSupported(false); return; }
    navigator.mediaDevices.enumerateDevices().then(all => {
      const outs = all.filter(d => d.kind === 'audiooutput');
      if (outs.length === 0) {
        setDevices([{ deviceId: 'default', label: 'System Default (MacBook Speakers)' }]);
      } else {
        setDevices(outs);
      }
  ...
  const applyDevice = async (id) => {
    setSelectedId(id);
    // setSinkId is Chrome/Edge only; store for future AudioContext use
    window.__audioSinkId = id;
  ...
  const playTest = () => {
    ...
    if (window.__audioSinkId && ctx.setSinkId) {
      ctx.setSinkId(window.__audioSinkId).catch(() => {});
    }
  ...
  return (
    <div className="modal-backdrop" onClick={onClose}>
      <div className="modal glass" style={{ width: 460 }} onClick={e => e.stopPropagation()}>
        <div className="m-head">
          ...
          <span>Audio Output Settings</span>
          <div style={{ flex: 1 }}/>
        ...
        <div className="m-body" style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          {!supported ? (
            <div style={{ padding: '10px 0', color: 'var(--ink-soft)', fontSize: 13 }}>
              Audio output selection requires Chrome or Edge. Your browser will use the system default output (MacBook speakers).
            </div>
          ) : (
            <>
              <div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginTop: 6 }}>
                  {devices.length === 0 && (
                    <div style={{ fontSize: 12, color: 'var(--ink-soft)' }}>
                      No output devices found — your MacBook speakers are being used.
                    </div>
                  )}
                  {devices.map(d => (
                    <label key={d.deviceId} style={{
                      ...
                      background: selectedId === d.deviceId
                        ? 'linear-gradient(180deg, #aef7cb, #4fd089 50%, #25a362 50%, #68e39d)'
                        : 'linear-gradient(180deg, rgba(255,255,255,.6), rgba(255,255,255,.2))',
                      border: `1px solid ${selectedId === d.deviceId ? '#1a6a3e' : 'rgba(255,255,255,.7)'}`,
                      fontSize: 13, fontWeight: selectedId === d.deviceId ? 700 : 400,
                      color: selectedId === d.deviceId ? '#0a2a18' : 'var(--ink)',
                      transition: 'all .15s',
                    }} onClick={() => applyDevice(d.deviceId)}>
                      <svg width="16" height="16" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5">
                        <path d="M2 5h4l3-3v12l-3-3H2V5z"/>
                        <path d="M11 6a3 3 0 010 4" opacity={selectedId === d.deviceId ? 1 : .4}/>
                        <path d="M13 4a5.5 5.5 0 010 8" opacity={selectedId === d.deviceId ? .6 : .2}/>
                      </svg>
                      {d.label || `Output ${d.deviceId.slice(0, 8)}`}
                      {selectedId === d.deviceId && <span style={{ marginLeft: 'auto', fontSize: 11 }}>✓ Active</span>}
                    </label>
                  ))}
                </div>
              </div>
              <div style={{
                ...
                border: '1px solid rgba(255,255,255,.6)', borderRadius: 8, padding: '10px 12px',
                fontSize: 11.5, color: 'var(--ink-soft)', lineHeight: 1.6
              }}>
                <b style={{ color: 'var(--ink)' }}>MacBook speakers</b> are used by default. For headphones or external interfaces, connect the device and re-open this panel to see it appear.
              </div>
            </>
          )}
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <button className={`btn${testPlaying ? ' primary' : ''}`} onClick={playTest} disabled={testPlaying}>
              <svg width="12" height="12" viewBox="0 0 12 12" fill="none">
                <path d="M2 1.5L10 6L2 10.5V1.5Z" fill="currentColor"/>
              </svg>
              {testPlaying ? 'Playing…' : 'Test Tone (A4)'}
            </button>
            <span style={{ fontSize: 11, color: 'var(--ink-soft)' }}>Plays a 440 Hz sine wave to verify output</span>
          </div>
        </div>
...
function stopPreview() {
  try { _previewNode && _previewNode.stop(); } catch(_) {}
  _previewNode = null;
}
...
function previewSample(name) {
  ...
  if (window.__audioSinkId && _previewCtx.setSinkId) {
    _previewCtx.setSinkId(window.__audioSinkId).catch(() => {});
  }
  // Generate a tone character based on sample name hash
  const hash = name.split('').reduce((a, c) => a + c.charCodeAt(0), 0);
  const freqs = [130.8, 146.8, 164.8, 174.6, 196, 220, 246.9, 261.6, 293.7, 329.6, 349.2, 392, 440, 493.9, 523.3];
  const freq = freqs[hash % freqs.length];
  const isDrum = /kick|snare|hat|clap|tom|perc|rim|snap/i.test(name);
  const isBass = /bass|sub|808/i.test(name);

  const osc = _previewCtx.createOscillator();
  const gain = _previewCtx.createGain();
  osc.type = isDrum ? 'sawtooth' : isBass ? 'triangle' : 'sine';
  osc.frequency.setValueAtTime(isDrum ? freq * 0.5 : freq, _previewCtx.currentTime);
  if (isDrum) osc.frequency.exponentialRampToValueAtTime(20, _previewCtx.currentTime + 0.15);
  gain.gain.setValueAtTime(0.35, _previewCtx.currentTime);
  gain.gain.exponentialRampToValueAtTime(0.001, _previewCtx.currentTime + (isDrum ? 0.3 : 1.2));
  osc.connect(gain);
  gain.connect(_previewCtx.destination);
  osc.start();
  osc.stop(_previewCtx.currentTime + (isDrum ? 0.3 : 1.2));
  _previewNode = osc;
}
...
function PinScreen({ onUnlock }) {
  ...
  const press = React.useCallback((d) => {
    if (unlocking || pin.length >= 3) return;
    setPin(p => p + d);
    setError(false);
  }, [pin, unlocking]);
  ...
  // Auto-submit once 3 digits are entered
  React.useEffect(() => {
    if (pin.length === 3) submit();
  }, [pin]);
